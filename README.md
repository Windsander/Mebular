...
[![Tests](https://img.shields.io/badge/Tests-336%20passed-brightgreen)](#项目状态)
...
除这四点之外，还带了 X25519 + AES-256-GCM 的加密信道、可用用户主密钥静态加密落盘（HKDF 派生 + AES-256-GCM，落盘密文、内存明文）、可选本地 embedding 语义召回（缺包自动降级关键词并告警）、Hermes 的七方法 Provider 和幂等导入、可选的 libp2p 真实网络（含 circuit relay v2 跨网段寻址与手动 multiaddr 降级），以及一套带覆盖率门槛的测试。
...
| 项目 | 情况 |
|------|------|
| 测试 | 44 个套件、336 条用例全绿，覆盖单元、双设备端到端、四端互通和故障注入 |
| 覆盖率 | 行 90.4%、分支 77.1%，全库门槛 85/65，关键文件另有底线 |
