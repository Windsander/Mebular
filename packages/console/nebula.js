...
// Orthographic projection, restrained color palette, soft structures
  ...
  Nebula.prototype._resize = function(){
    var __rect = this.canvas.getBoundingClientRect();
    this.W = this.canvas.width = Math.max(1, Math.floor(__rect.width || window.innerWidth));
    this.H = this.canvas.height = Math.max(1, Math.floor(__rect.height || window.innerHeight));
    this._createMistLayer();
  };
